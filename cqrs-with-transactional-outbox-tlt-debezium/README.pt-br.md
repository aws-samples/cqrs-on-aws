...
Depois de inserir um evento no Aurora, o Debezium lê o log de transações da outbox table e publica os eventos em um tópico
do Amazon MSK, que é lido por um pipe do EventBridge que publica os eventos em um tópico do SNS. O SNS então entrega os
eventos em duas filas, uma das quais é lida por um Lambda que atualiza o Redis, com os dados mais recentes do cliente
relacionados a cada evento.
...
11. Para remover a infraestrutura que foi provisionada para não incorrer em custos, algumas etapas são necessárias. Como a infraestrutura criada é relativamente complexa e alguns recursos precisam ser criados após a criação da infraestrutura inicial, para limpar os recursos, será necessário seguir algumas etapas. Para excluir a infraestrutura criada, no console, acesse o Amazon MSK. Em seguida, no MSK Connect, vá até "Connectors", selecione o conector "KafkaOrderEventConnector" e exclua-o. Em seguida, vá para "Custom plugins", selecione o plugin "debezium-plugin" e exclua-o. Em seguida, acesse o Amazon CloudFormation, Stacks e clique no radio button da stack "cqrsOnAws". Exclua a stack. Essa exclusão levará aproximadamente 30 minutos. Provavelmente não será possível excluir a stack inteira. Se isso acontecer, acesse o Amazon EC2 e, em "Network & Security", vá até "Network Interfaces". Exclua as duas interfaces restantes selecionando-as e clicando em "Delete" no menu "Actions". Em seguida, volte para o CloudFormation, selecione a stack "cqrsOnAws" e clique em "Retry Delete". Em seguida, escolha "Force delete this entire stack" e clique em "Delete".