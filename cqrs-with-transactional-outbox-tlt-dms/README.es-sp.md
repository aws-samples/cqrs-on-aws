...
Esta parte describe cómo ter a infraestrutura explorada no blog post [CQRS en AWS: Sincronizando los Servicios de Command y Query con el Estándar Transactional Outbox, la Técnica Transaction Log Tailing y el Amazon Database Migration Service](https://aws.amazon.com/es/blogs/aws-spanish/cqrs-en-aws-sincronizando-los-servicios-de-command-y-query-con-el-estandar-transactional-outbox-la-tecnica-transaction-log-tailing-y-el-amazon-database-migration-service)
...
foi autorizado, etc.). Tras insertar un evento en Aurora, Amazon DMS lee el registro de transacciones de la bandeja de salida
y publica los eventos en una transmisión de Amazon Kinesis Data Streams, que lee un pipe de EventBridge que publica
los eventos en un tema de SNS. A continuación, SNS distribuye los eventos en dos colas, una de las cuales es leída por una
función Lambda que actualiza Redis con los datos de clientes más recientes relacionados con cada evento.
...
Node.js y npm](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm).  Para instalar la CLI de AWS, siga las instrucciones de la página [Instalar o actualizar a la última
versión de la CLI de AWS](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html). Para instalar la CLI de AWS CDK, siga las instrucciones de la página [AWS CDK Toolkit](https://docs.aws.amazon.com/cdk/v2/guide/cli.html).
Por último, para instalar el lenguaje de programación Python, siga las instrucciones de la [Página de descargas de Python](https://www.python.org/downloads).
...
2. En una terminal, navegue hasta el directorio "cqrs-with-transactional-outbox-tlt-dms".
...
11. Para eliminar la infraestructura que se aprovisionó a fin de no incurrir en costos, es necesario tomar algunas medidas. Dado que iniciar la tarea de migración de Amazon DMS es un paso que se da después de crear la infraestructura, tendrá que seguir algunos pasos. Para eliminar la infraestructura creada, en la consola, vaya a la página de Amazon DMS. Luego, en el dashboard, debería haber una tarea activa. Navega hasta ella. En "Database migration tasks", selecciona la tarea en ejecución, elige "Delete" en el menú "Actions" y confirma. Navegue hasta la página de Amazon CloudFormation, elija la pila "cqrsOnAws, elija "Delete" y confirme. Esto tardará unos 40 minutos. A continuación, vaya a la página de Amazon EC2 y, en "Network & Security, vaya a "Network Interfaces". Elija la interfaz con la descripción "DMSNetworkInterface" y, en el menú "Actions", elija "Delete" y confirme. A continuación, vaya a la página de Amazon VPC. Elija la VPC cuyo nombre comience por "cqrsOnAws" y, en el menú "Actions", elija "Delete" y confirme. Por último, vuelva a la página de CloudFormation, elija la stack cuyo nombre comience por "cqrsOnAws, seleccione "Retry Delete", luego "Force delete this entire stack" y, por último, "Delete".